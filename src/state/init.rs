...
use hoppinworldruntime::{PlayerSettings, ObjectType, AllEvents, RemovalId};
...
impl dynamic::State<MyState, AllEvents> for InitState {
    fn on_start(&mut self, world: &mut World) {
        world.register::<ObjectType>();
        world.register::<Removal<RemovalId>>();
        world.add_resource(get_all_maps(&get_working_dir()));
        world.add_resource(AmbientColor(Rgba::from([0.1; 3])));
        world.add_resource(FutureProcessor::default());
        let hide_cursor = HideCursor { hide: false };
        world.add_resource(hide_cursor);
        ...
        world.add_resource(world_param);

        world.add_resource(player_settings);
    ...
    fn update(&mut self, world: &mut World) -> Trans<MyState> {
        Trans::Switch(MyState::Login)
        //Trans::Switch(MyState::MenuState)
    }
}
